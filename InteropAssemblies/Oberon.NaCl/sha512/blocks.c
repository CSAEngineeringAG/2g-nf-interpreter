...
const uint64 const_table_64[5 * 16] = {
...
int crypto_hashblocks(unsigned char *statebytes,const unsigned char *in,unsigned long inlen)
{
    ...
    while (inlen >= 128) {
        ...
        uint64 w[16];
        const uint64 * cptr = const_table_64;
            
        for (i = 0; i < 16; i++)
        {
            w[i] = load_bigendian(in);
            in += 8;
        }
        ...
        while (1)
        {
            for (i = 0; i < 16; i++)
            {
                T1 = h + Sigma1(e) + Ch(e,f,g) + *cptr++ + w[i];
    ...
    for (i = 0; i < 8; i++)
    {
        store_bigendian(statebytes, state[i]);
        statebytes += 8;
    }
